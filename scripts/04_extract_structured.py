#!/usr/bin/env python3
"""
04_extract_structured.py — Extract structured facility data from Form A Part 1 segments.

Reads data/segmented/{id}/form_a1.txt, sends chunks to Claude Sonnet API,
and saves structured JSON to data/structured/{id}_form_a1.json.

Documents are split at facility boundaries ("1. Name(s) of facility" and
language equivalents) into chunks of up to 4 000 characters each, then
processed in multiple API calls.

Use --form-g to extract Form G (vaccine production facilities) instead.
...
def main() -> None:
    ...
    parser.add_argument("--form-g", action="store_true",
                        help="Extract Form G (vaccine facilities) instead of Form A Part 1.")
    ...
    # Form G accepts all languages (template fields are consistent across languages)
    if args.form_g:
        eligible = [
            ...
            and (SEGMENTED_DIR / e["id"] / "form_g.txt").exists()
        ]
    else:
        eligible = [
            e for e in catalogue
            ...
            and not e.get("needs_ocr")
            and not e.get("is_amendment")
            and (SEGMENTED_DIR / e["id"] / "form_a1.txt").exists()
        ]

    suffix = "_form_g.json" if args.form_g else "_form_a1.json"
    ...
    last_t: list[float] = [0.0]   # mutable so sub-functions can update it

    n_ok = n_skipped = n_failed = 0
    total_facilities = total_calls = total_in = total_out = 0
    results: list[dict] = []

    desc = "Extracting Form G" if args.form_g else "Extracting"
    for entry in tqdm(targets, desc=desc, unit="doc", dynamic_ncols=True):
        result = process_entry_g(entry, client, last_t) if args.form_g else process_entry(entry, client, last_t)
        results.append(result)

        status = result.get("status", "")
        if status == "ok":
            n_ok          += 1
            fac_key = "vaccine_facilities" if args.form_g else "facilities"
            total_facilities += result.get(fac_key, 0)
